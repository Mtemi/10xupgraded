...
type SignalMark = {
  timeSec: number;                       // unix seconds
  price: number;
  side: 'BUY' | 'SELL';
  tag?: string;                          // enter_tag / exit_tag if present
  pair: string;
};
...
export function TradingViewChart({
  ...
  // store signal marks collected from history + realtime
  const signalMarksRef = useRef<SignalMark[]>([]);
  const signalKeySetRef = useRef<Set<string>>(new Set()); // de-duplicate by (pair,time,side)

  // NEW: remember last flag values to detect 0→1 edges
  const lastSignalRef = useRef<{ enter: number; exit: number; tsSec: number } | null>(null);
  ...
  // helper to normalize flags to 0/1
  const norm = (v: any) => (Number(v) > 0 ? 1 : 0);

  // helper to add a unique signal mark (from history or realtime)
  const addSignalMark = (mark: SignalMark) => {
    if (!mark || !Number.isFinite(mark.timeSec) || !Number.isFinite(mark.price)) return;
    if (mark.pair !== selectedPair) return; // only draw for current pair
    // de-dupe by (pair, bar time, side) — exclude price so the same bar won't duplicate during updates
    const key = `${mark.pair}|${mark.timeSec}|${mark.side}`;
    if (signalKeySetRef.current.has(key)) return;
    signalKeySetRef.current.add(key);
    signalMarksRef.current.push(mark);
  };
  ...
  function createDatafeed(trades: Trade[]) {
    ...
    return {
      ...
      getBars: async (_symbolInfo: any, resolution: string, periodParams: any, onHistory: any, _onError: any) => {
        try {
          const { from, to, countBack } = periodParams; // seconds
          ...
          if (!user) { onHistory([], { noData: true }); return; }
          ...
          if (!resp.ok) { console.error('[TradingViewChart] getBars HTTP', resp.status); onHistory([], { noData: true }); return; }

          const text = await resp.text();
          if (!text || text.trim() === '') { onHistory([], { noData: true }); return; }

          let json: any;
          try { json = JSON.parse(text); } catch { onHistory([], { noData: true }); return; }
          if (!json.columns || !json.data || !json.data.length) { onHistory([], { noData: true }); return; }
          ...
          // indices for signals/tags if present
          const iEnterLong = cols.indexOf('enter_long');
          const iExitLong  = cols.indexOf('exit_long');
          const iEnterTag  = cols.indexOf('enter_tag');
          const iExitTag   = cols.indexOf('exit_tag');
          ...
          // collect BUY/SELL marks only on 0→1 transitions
          if (iEnterLong >= 0 || iExitLong >= 0) {
            let prevEnter = 0;
            let prevExit  = 0;
            for (const b of inRange) {
              const row = b.__raw as any[];
              const tSec = Math.floor(b.time / 1000);
              const enterF = iEnterLong >= 0 ? norm(row[iEnterLong]) : 0;
              const exitF  = iExitLong  >= 0 ? norm(row[iExitLong])  : 0;

              if (enterF === 1 && prevEnter !== 1) {
                addSignalMark({
                  timeSec: tSec,
                  price: b.close,
                  side: 'BUY',
                  tag: iEnterTag >= 0 ? String(row[iEnterTag] ?? '') : undefined,
                  pair: selectedPair
                });
              }
              if (exitF === 1 && prevExit !== 1) {
                addSignalMark({
                  timeSec: tSec,
                  price: b.close,
                  side: 'SELL',
                  tag: iExitTag >= 0 ? String(row[iExitTag] ?? '') : undefined,
                  pair: selectedPair
                });
              }
            ...
            // remember last state for realtime edge detection
            if (inRange.length) {
              const last = inRange[inRange.length - 1].__raw as any[];
              const lastTimeSec = Math.floor(inRange[inRange.length - 1].time / 1000);
              lastSignalRef.current = {
                enter: iEnterLong >= 0 ? norm(last[iEnterLong]) : 0,
                exit:  iExitLong  >= 0 ? norm(last[iExitLong])  : 0,
                tsSec: lastTimeSec
              };
            }
          }

          onHistory(inRange.map(({ __raw, ...bar }) => bar), { noData: inRange.length === 0 });

          setTimeout(drawMarkers, 0);
        } catch (err) {
      ...
      subscribeBars: (_symbolInfo: any, _resolution: string, onRealtimeCallback: (bar: any) => void, _subscriberUID: string) => {
        ...
        // reset last signal state on new subscription
      ...
      getMarks: (_symbolInfo: any, startTime: number, endTime: number, onDataCallback: (marks: any[]) => void) => {
        ...
        // From collected signal marks (history + realtime)
        for (const sm of signalMarksRef.current) {
          if (sm.pair !== selectedPair) continue;
          if (sm.timeSec >= startTime && sm.timeSec <= endTime) {
            marks.push({
              id: id++,
              time: sm.timeSec,
              color: sm.side === 'BUY' ? 'green' : 'red',
              label: sm.side === 'BUY' ? 'B' : 'S',
              labelFontColor: '#fff',
              minSize: 8,
              text: [`${sm.side} @ ${sm.price}`, sm.tag ? String(sm.tag) : '']
            });
          }
        }
  ...
  const drawMarkers = () => {
    ...
    try {
        ...
        try {
          chart.createMultipointShape(
            [{ time, price: trade.open_price }],
            {
              shape: 'arrow_up',
              text: `Buy`,
              overrides: { backgroundColor: '#00AA00', textColor: '#FFFFFF', fontsize: 12, bold: true },
      ...
      for (const sm of signalMarksRef.current) {
        ...
        try {
          chart.createMultipointShape(
            [{ time: sm.timeSec, price: sm.price }],
            {
              shape: sm.side === 'BUY' ? 'arrow_up' : 'arrow_down',
              text: sm.tag ? `${sm.side}\n${sm.tag}` : sm.side,
              overrides: {
                backgroundColor: sm.side === 'BUY' ? '#00AA00' : '#CC0000',
                textColor: '#FFFFFF', fontsize: 12, bold: true
              },
              disableSelection: true, disableSave: true, disableUndo: true,
            }
  ...
  useEffect(() => {
    ...
    if (tvWidgetRef.current && currentTimeframeRef.current !== tvInterval) {
      // clear signals on timeframe change
      signalMarksRef.current = [];
      signalKeySetRef.current.clear();
      lastSignalRef.current = null;
  ...
  // --- WebSocket (LIVE) bridge (unchanged logic except edge detection/de-dupe for signals) ---
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      ws.onmessage = (evt) => {
        ...
        try {
          ...
          // Parse FreqUI dataframe wrapper
          const df = payload?.df;
          if (!candle && df?.__type__ === 'dataframe' && typeof df.__value__ === 'string') {
            try {
              ...
              if (last && cols.length) {
                ...
                // edge-detected signals (0→1 only), de-duped by (pair,time,side)
                const tsForMarkSec = (() => {
                  const ts = Number(last[iDate]);
                  const tsMs = ts > 1e12 ? ts : ts * 1000;
                ...
                const enterF = iEnterLong >= 0 ? norm(last[iEnterLong]) : 0;
                const exitF  = iExitLong  >= 0 ? norm(last[iExitLong])  : 0;
                const prev = lastSignalRef.current || { enter: 0, exit: 0, tsSec: 0 };

                if (enterF === 1 && prev.enter !== 1) {
                  addSignalMark({
                    timeSec: tsForMarkSec,
                    price: Number(last[iC] ?? last[iO]),
                    side: 'BUY',
                    tag: iEnterTag >= 0 ? String(last[iEnterTag] ?? '') : undefined,
                    pair: selectedPair
                  });
                }
                if (exitF === 1 && prev.exit !== 1) {
                  addSignalMark({
                    timeSec: tsForMarkSec,
                    price: Number(last[iC] ?? last[iO]),
                    side: 'SELL',
                    tag: iExitTag >= 0 ? String(last[iExitTag] ?? '') : undefined,
                    pair: selectedPair
                  });
                }
                lastSignalRef.current = { enter: enterF, exit: exitF, tsSec: tsForMarkSec };
              }
            } catch (err) {
          ...
          if (!candle && Array.isArray(payload?.columns) && Array.isArray(payload?.data) && payload.data.length) {
            ...
            const iEnterLong = idx('enter_long'), iExitLong = idx('exit_long');
            const iEnterTag  = idx('enter_tag'),  iExitTag  = idx('exit_tag');
            ...
            lastSignalRef.current = { enter: enterF, exit: exitF, tsSec: tSec };
          }
