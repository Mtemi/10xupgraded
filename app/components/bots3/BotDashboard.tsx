...
const BotDashboard: React.FC<BotDashboardProps> = () => {
  ...
  const fetchProfit = async (strategy = strategyName) => {
    ...
    try {
      ...
      const apiUsername = 'meghan';
      const apiPassword = session.user.id;
      const resp = await fetch(`/user/${strategy}/api/v1/profit`, {
        headers: {
          'Authorization': 'Basic ' + btoa(`${apiUsername}:${apiPassword}`)
        }
      });
  ...
  const fetchBalance = async (strategy = strategyName) => {
    ...
    try {
      ...
      const apiUsername = 'meghan';
      const apiPassword = session.user.id;
      const resp = await fetch(`/user/${strategy}/api/v1/balance`, {
        headers: { 
          'Authorization': 'Basic ' + btoa(`${apiUsername}:${apiPassword}`) 
        }
      });
  ...
  const fetchDailyProfit = async (strategy = strategyName) => {
    ...
    try {
      ...
      const resp = await fetch(`/user/${strategy}/api/v1/daily`, {
        headers: {
          'Authorization': 'Basic ' + btoa(`${apiUsername}:${apiPassword}`)
        }
      });
      ...
      console.log('[BotDashboard] Normalized Daily Profit:', normalized);
  ...
  // Render chart tab
  const renderChart = () => {
    console.log('🔍 renderChart openTrades:', openTrades);
    console.log('🔍 renderChart closedTrades:', closedTrades);
    return (
          ...
          {selectedPair && (
            <button
              ...
              className={classNames(
                ...
                isActionLoading || botStatus !== 'running'
